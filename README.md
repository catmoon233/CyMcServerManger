...
- 📱 **响应式设计** - 支持移动端访问
- 🚫 **输出屏蔽** - 在Web界面中屏蔽特定服务器的日志显示
- 📋 **模板复制** - 从模板服务器选择性复制配置到新服务器

## 新功能快速指南

### 屏蔽服务器输出

在Web界面中隐藏特定服务器的控制台输出，但日志文件仍会保存。

```bash
block add server1        # 屏蔽 server1 的输出
block toggle server1     # 切换屏蔽状态
block list              # 查看所有被屏蔽的服务器
```

**详细指南：** [快速入门指南](QUICK_START_GUIDE.md)

### 模板复制

从一个服务器选择性地复制文件夹（mods、config、world等）到另一个服务器。

```bash
# 从模板复制默认文件夹（mods, config, world, plugins）
copy template source_server target_server

# 只复制指定的文件夹
copy template source_server target_server --folders mods,config

# 允许覆盖已存在的文件夹
copy template source_server target_server --folders mods --overwrite
```

**详细指南：** [控制台命令指南](CONSOLE_COMMANDS_GUIDE.md) | [集成说明](INTEGRATION_SUMMARY.md)
...
2. **启动/停止** - 一键启动或停止服务器
3. **屏蔽输出** - 在Web界面中隐藏特定服务器的日志显示
4. **复制模板** - 从一个服务器选择性地复制文件夹到另一个服务器
5. **控制台** - 实时查看服务器控制台输出和发送命令
6. **创建服务器** - 快速创建新的服务器实例
7. **删除服务器** - 删除服务器配置和文件
8. **实时监控** - 服务器状态自动刷新
...
- `block add <server>` - 屏蔽服务器控制台输出（Web界面中）
- `block remove <server>` - 取消屏蔽服务器输出
- `block toggle <server>` - 切换服务器屏蔽状态
- `block list` - 查看所有被屏蔽的服务器
- `copy template <source> <target>` - 从模板服务器复制指定文件夹到目标服务器
- `copy <source> <target>` - 完整复制一个服务器
...
- ✨ 新增屏蔽服务器输出功能 - 在Web界面中隐藏特定服务器的日志
- ✨ 新增模板复制功能 - 从服务器模板选择性复制mods、config、world等文件夹
- 🔧 增强Console命令界面 - 改进block和copy命令的用户体验
- 📚 完整文档更新 - 添加快速入门指南和集成说明文档
