...
public class StartCommand extends AnnotatedCommand {
    ...
    public boolean execute(String[] args) {
        try {
            ...
            if (servers.isEmpty()) {
                Logger.println(ConsoleColor.colorize(ConsoleColor.RED, "错误: 没有可用服务器"));
                return true;
            }
            
            // 显示服务器列表
            Logger.println(ConsoleColor.colorize(ConsoleColor.BRIGHT_GREEN, "可用服务器："));
            ...
            try {
                ...
                Logger.println(ConsoleColor.colorize(ConsoleColor.RED, "错误: 请输入有效的数字"));
                return true;
            }
            
            if (choice == -1) { // 用户选择返回
                Logger.println(ConsoleColor.colorize(ConsoleColor.YELLOW, "返回主菜单"));
                return true;
            }
            ...
            Logger.println(ConsoleColor.colorize(ConsoleColor.BRIGHT_GREEN, 
                "\n选择启动模式:"));
            Logger.println("1. " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, "核心模式 (推荐)") + 
                             " - " + ConsoleColor.colorize(ConsoleColor.BRIGHT_BLACK, "标准Minecraft服务器启动"));
            Logger.println("2. " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, "模组包模式") + 
                             " - " + ConsoleColor.colorize(ConsoleColor.BRIGHT_BLACK, "适用于Forge/Fabric模组包"));
            Logger.println("3. " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, "基础模式") + 
                             " - " + ConsoleColor.colorize(ConsoleColor.BRIGHT_BLACK, "最小化参数启动"));
            Logger.println("4. " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, "基础模式(修复版)") + 
            ...
            if (launchMode < 1 || launchMode > 5) {
                Logger.println(ConsoleColor.colorize(ConsoleColor.RED, "错误: 无效的启动模式"));
                return true;
            }
            ...
            Logger.println(ConsoleColor.colorize(ConsoleColor.BRIGHT_YELLOW, 
                "\n即将启动服务器:"));
            Logger.println("- 服务器: " + ConsoleColor.colorize(ConsoleColor.BRIGHT_CYAN, selectedServer.getName()));
            Logger.println("- 版本: " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, selectedServer.getVersion()));
            Logger.println("- 启动模式: " + getLaunchModeName(actualLaunchMode));
            if (javaPath != null) {
                Logger.println("- Java路径: " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, javaPath));
            }
            if (jvmArgs != null) {
                Logger.println("- JVM参数: " + ConsoleColor.colorize(ConsoleColor.BRIGHT_WHITE, jvmArgs));
            }
            if (serverArgs != null) {
            ...
            if ("n".equals(confirm) || "no".equals(confirm)) {
                Logger.println(ConsoleColor.colorize(ConsoleColor.YELLOW, "启动已取消"));
                return true;
            }
